...
- High confidence  (≥ HIGH_CONF=0.75) → auto-accepted
- Medium confidence (≥ INF_CONF=0.50) → accepted with warning flag
- Low confidence   (< INF_CONF=0.50)  → routed to human review queue
"""
...
_review_queue: list[dict] = []   # pending low-confidence cases
_audit_log:    list[dict] = []   # full audit trail of every request
...
class InferenceResponse(BaseModel):
    ...
    request_id:      str   = ""
    # ── Confidence routing fields (BONUS) ──
    confidence:      float = 0.0          # mean entity-level confidence 0-1
    routing:         str   = "auto"       # "auto" | "flagged" | "review"
    review_queue_id: Optional[str] = None # set when routed to review
...
class ReviewDecision(BaseModel):
    ...
    reviewer:   str
    correction: Optional[dict] = None   # corrected entity dict if action=="correct"
...
def compute_mean_confidence(preds: list[dict]) -> float:
    """Mean prediction confidence across non-O tokens."""
...
# ─────────────────── Confidence Routing Logic (BONUS) ─────────────
def route_response(
    request_id: str,
    parsed: ParsedResume,
    confidence: float,
    ...
    Route the result based on confidence:
      ≥ HIGH_CONF  → routing="auto"    (accepted automatically)
      ≥ INF_CONF   → routing="flagged" (returned but marked uncertain)
      < INF_CONF   → routing="review"  (placed in human review queue)
...
async def parse_pdf(file: UploadFile = File(...)):
    ...
    try:
        ...
        try:
            s3_key = s3_service.upload_resume(raw, file.filename)
            _audit("UPLOADED_TO_S3", request_id, s3_key=s3_key)
        except Exception as upload_err:
            logger.error(f"S3 Upload failed for {file.filename}: {upload_err}")
            _audit("S3_UPLOAD_FAILED", request_id, error=str(upload_err))
        # ---------------------------------
    ...
    confidence = compute_mean_confidence(preds)
    parsed     = ents_to_schema(ents)
    response   = route_response(request_id, parsed, confidence, source="pdf")
    response.latency_ms = round(ms, 1)
...
def get_review_queue(status: Optional[str] = None):
    ...
    Optional filter: ?status=pending | approved | corrected
    """
    items = _review_queue
    if status:
...
def get_review_item(queue_id: str):
    """Fetch a single review-queue item by its queue_id."""
...
def decide_review_item(queue_id: str, decision: ReviewDecision):
    """
    Human reviewer approves or corrects a queued result.
    action = "approve"  → mark as approved, entities unchanged
    action = "correct"  → mark as corrected, store correction dict
    """
    item = next((q for q in _review_queue if q["queue_id"] == queue_id), None)
    if not item:
        raise HTTPException(status_code=404, detail="Queue item not found")
    ...
    if decision.action == "correct" and not decision.correction:
        raise HTTPException(status_code=400, detail="correction payload required when action='correct'")

    now = datetime.now(timezone.utc).isoformat()
    item["status"]      = "approved" if decision.action == "approve" else "corrected"
...
def get_audit_log(request_id: Optional[str] = None, limit: int = 100):
    ...
    Optional filter: ?request_id=<uuid>
    Optional limit:  ?limit=100  (default 100, max 1000)
    """
...
@app.get("/audit/stats")
def audit_stats():
    """High-level stats: total requests, routing breakdown, queue sizes."""
